...
class Model(torch.nn.Module):
    ...
    def save(self, path):
        ...
        torch.save(self.model_state.state_dict(), path + "model_state.pt")
        torch.save(self.model_reward.state_dict(), path + "model_reward.pt")


    def load(self, path):       
        self.model_features.load_state_dict(torch.load(path + "model_features.pt", map_location = self.device))
        self.model_state.load_state_dict(torch.load(path + "model_state.pt", map_location = self.device))
        self.model_reward.load_state_dict(torch.load(path + "model_reward.pt", map_location = self.device))
